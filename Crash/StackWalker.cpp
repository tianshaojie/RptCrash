	...
	while (StackWalk64(imageType, m_hProcess, hThread, &sf, &c, NULL, SymFunctionTableAccess64, SymGetModuleBase64, NULL))
	{
		...
		if (sf.AddrPC.Offset != 0)
		{
			...
			}else
			{
				...
				this->OutputString(_T("Call SymGetSymFromAddr64 ,Address %08x Error:%08x"), sf.AddrPC.Offset, GetLastError());
				continue;
			}
			...
			{
				this->OutputString(_T("Call SymGetLineFromAddr64 ,Address %08x Error:%08x"), sf.AddrPC.Offset, GetLastError());
				continue;
			}
			
			//这里为了将获取函数信息失败的情况与正常的情况一起输出，防止用户在查看时出现误解
			this->OutputString(_T("%08llx:%s [%s][%ld]"), pCallStack->szFncAddr, pCallStack->undFullName, pCallStack->szFileName, pCallStack->uFileNum);
			if (NULL == pHead)
			{
				pHead = pCallStack;
				pTail = pHead;
